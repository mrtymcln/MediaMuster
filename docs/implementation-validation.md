...
The complete app built with Qt 6.5.3 and C++17 as a universal macOS binary containing Apple Silicon and Intel code. The full CTest run passed **28 of 28 test executables** in 47.32 seconds. QtTest reported 567 passing checks, including setup and cleanup checks, and no failures. Three individual checks skipped: two optional external-toolkit tests without their environment variables, and one case-sensitive directory check on the current case-insensitive filesystem.

The external-toolkit checks were subsequently included in the independent memory-error validation. Six reader suites built with AddressSanitizer and UndefinedBehaviorSanitizer passed **212 checks with no failures, skips or sanitizer diagnostics**:
...
This used a separate native Apple Silicon Debug build with `-fsanitize=address,undefined -fno-omit-frame-pointer -fno-sanitize-recover=all`. It exercised the existing real Avid corpus, constructed malformed/boundary cases, all 65 original-toolkit OMF container specimens and the toolkit semantic comparisons. It was deterministic corpus/test validation, not fuzzing. Prebuilt Qt and system libraries were not rebuilt with instrumentation.
...
The rebuilt universal application passed signature verification. A fresh complete CTest run passed **28 of 28 suites in 49.29 seconds**, with **632 passing QtTest results**, zero failures and the same three individual skips described above. Six freshly rebuilt sanitizer suites then passed **252 results**, zero failures/skips and no sanitizer diagnostics: PMR 100, Bento 20, MXF 56, OMF 35, MDB 31 and OMF UID 10. Both external-toolkit checks were enabled in that run. The all-media probe also ran with ASan/UBSan without diagnostics.
