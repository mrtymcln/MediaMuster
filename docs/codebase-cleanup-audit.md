...
The reported Windows `max` macro compile clash is corrected, and native transient-error classification controls bounded copy retries. Windows CI subsequently compiled the application and all test targets; its single failing Move-test assertion and correction are recorded in [implementation validation](file-operations-native-api-validation.md#windows-ci-move-test-follow-up). A new Windows run must confirm the test correction. The accepted cleanup and renames are implemented; explicit CMake source lists and `RevealInFinder` are retained. Deeper scanner/runner state decomposition remains the later pass after Windows/NEXIS baseline testing, as sequenced above.
