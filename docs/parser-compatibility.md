...
| MXF properties | The [BBC libMXF data-model definitions](https://github.com/BBC-archive/libMXF/tree/main/mxf) supply the property identifiers used to resolve each file's Primer Pack. The selected identifiers and attribution are in `src/mxfproperties.h`; complete redistribution notices are embedded in About → Licenses. |
