...
| Media family | The accepted managed tree selects MXF or OMF, independently of unrelated database IDs. Each family admits only its own supported suffixes. Current, complete database records retain the no-header-read fast path. |
| OMF project and bin matching | All objects sharing the linked source ID contribute project metadata in file order. Legacy bin IDs retain their original byte order, so distinct OMF clips cannot become aliases of each other. |
...
- The user reports successful field testing with real Avid systems and NEXIS/NAS storage. Automated shared-storage cases use local directories; these do not independently establish simultaneous-writer behaviour on every server. File modification dates provide a practical cache check, not proof that another application cannot change a file during scanning.
