...
All seven findings below have executable reproductions in [repro.cpp](repro.cpp), compiled against the current production sources. Output is in [repro-output.txt](repro-output.txt). These are constructed regression cases, not a claim that the user's existing corpus contains the triggers.
...
All reject the input only after the full allocation. Fresh-run logs [small MDB](rss-small-mdb.txt), [large MDB](rss-large-mdb.txt), and [large PMR](rss-large-pmr.txt) contain repeat measurements; allocator/load differences may change RSS slightly.
...
```sh
cmake -S /Users/martymclean/Developer/MediaMuster/docs/reviews/2026-09-06-current-code/evidence/parsers -B /tmp/mediamuster-review-20260906-preserved/parsers-build -DCMAKE_PREFIX_PATH=/Users/martymclean/Qt/6.5.3/macos
