...
The subsequent Unfinished Business UI change builds on both platforms and passes
all 37 operation-UI cases on Mac and Windows, with no failures or skips.
[Windows run 35083550697](https://github.com/mrtymcln/MediaMuster/actions/runs/35083550697)
tests diagnostic commit `b6f9cd78952797a4c6e9f771d01f038790d48300` and builds the
full application plus the affected UI suite. Added coverage checks the single
menu command, merged job choices, the safe default Close action, and recovery
before a waiting new operation. The dialog was also rendered and inspected in
Mac and Windows widget styles using example data.

The later wording adjustment replaces the footer Close button with the standard
Cancel button and removes the introductory sentence. The dismissal behavior is
unchanged; the rebuilt Mac application passes the same 37 operation-UI cases.

The subsequent layout refinement removes the footer button and original-location
group heading, uses Job and Files remaining, and silently skips empty recovery
dialogs. The rebuilt Mac application verifies its signature and passes all 37
operation-UI cases. Updated checks confirm window close and Escape preserve
recovery, and Enter on each action button neither starts work nor closes the dialog.
The refined dialog was rendered and inspected using the native macOS Qt style.

The approved conditional Cancel Job explanations and table-style dates also pass
the same 37 Mac operation-UI cases. Both explanation variants, date locations,
and the shortened layout were rendered and visually checked with dummy data.
