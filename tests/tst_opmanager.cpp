...
// synchronous runner tests can't reach: reading the ledger while a copy
...
// run's ledger now SURVIVES (it is the undo candidate) where v1 pruned
// it.
...
private slots:
	...
	void copy_replace_ledgers_parked_path_while_still_in_flight();
	void copy_replace_midCopyFailure_restores_original();
	void copy_replace_cancel_restores_and_is_not_counted_failed();
	void copy_replace_strandedRestore_keepsDirtyLedger();
...
void TestOpManager::cleanup()
{
	...
	// v2 retention keeps finished ledgers around — wipe them between
	// tests or every scan-based assertion sees its predecessors' runs.
	const QDir oplog(m_oplogDir.path());
	for (const QString &name : oplog.entryList({"oplog-*.jsonl"}, QDir::Files))
		QFile::remove(oplog.filePath(name));
}

void TestOpManager::writeFile(const QString &path, const QByteArray &contents)
{
...
QByteArray TestOpManager::readFile(const QString &path)
{
	QFile f(path);
...
void TestOpManager::items_from_mediafiles_carry_scan_claims()
{
	...
	// what the runner cross-checks and what makes ledgers readable.
	QCOMPARE(items[0].mobId, mf.mobId);
	QCOMPARE(items[0].masterMobId, mf.masterMobId);
	QCOMPARE(items[0].clipName, mf.clipName);

	// The dialog's preview statics keep MediaManager's shapes.
...
void TestOpManager::copy_run_writes_its_plan_before_the_first_op()
{
	...
	const QVector<OpLedger::Record> recs = OpLedger::scan(m_oplogDir.path());
	...
	// The v2 flip: a finished run KEEPS its ledger — it is the undo
	// candidate now (v1 pruned it here, which is why undo had nothing
	// to work with).
	const auto after = OpLedger::scan(m_oplogDir.path());
	QCOMPARE(after.size(), 1);
	QVERIFY(after.first().complete);
	QVERIFY(OpLedger::latestUndoable(m_oplogDir.path()).has_value());
}

void TestOpManager::copy_replace_ledgers_parked_path_while_still_in_flight()
{
	...
	// The finished ledger records where the replaced original went.
	const auto after = OpLedger::scan(m_oplogDir.path());
	QCOMPARE(after.size(), 1);
...
void TestOpManager::copy_replace_strandedRestore_keepsDirtyLedger()
{
	// The finding-3 scenario end-to-end: a Replace copy fails AND the
	// rollback can't put the parked original back. The ledger must
	...
	// The dirty record pinned the ledger, and a dirty ledger is
	// recovery's business — never an undo candidate.
	const QStringList ledgers =
		QDir(m_oplogDir.path()).entryList({"oplog-*.jsonl"}, QDir::Files);
	bool sawDirty = false;
	for (const QString &name : ledgers)
		if (readFile(m_oplogDir.path() + "/" + name).contains("\"dirty\":true"))
			sawDirty = true;
	QVERIFY2(sawDirty, "the ledger must survive with the rollback-incomplete marker");
	QVERIFY(!OpLedger::latestUndoable(m_oplogDir.path()).has_value());
}
