...
void TestMdbParser::master_usage_conflicts_and_widths_stay_unknown()
{
	QTemporaryDir temp;
	for (bool omf2 : {false, true})
		for (bool big : {false, true})
			for (int next : {-1, 7, 1, 4, 9, 99, -2, -3})
			{
				...
				w.setImmediate(1, omf2 ? "OMFI:HEAD:ByteOrder" : "OMFI:ByteOrder", QByteArray(big ? "MM" : "II", 2));
				const QByteArray id = w.word(42) + w.word(1) + w.word(7);
