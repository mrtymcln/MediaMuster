...
public:
	...
	quint32 propertyId(const char *name)
	{
		if (!names.contains(name))
			names.insert(name, nextProperty++);
	...
	QByteArray build() const
	{
		...
		auto append = [&](const Row &r)
		{
			...
			{
				toc += char(1);
				toc += word(r.object) + word(r.property) + word(r.property == 31 ? 32 : 0);
				toc += char(4);
				toc += word(1);
				if (r.references)
				{
					toc += char(15);
					toc += word(r.references);
				}
			}
			if (r.immediate)
			{
		...
		for (auto it = names.cbegin(); it != names.cend(); ++it)
			append({it.value(), 24, it.key() + '\0', false, false, 0});
...
private:
	struct Row
	{
		quint32 object, property;
		QByteArray data;
		bool immediate, continued;
		quint32 references;
	};
	bool big;
