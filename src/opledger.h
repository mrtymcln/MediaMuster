...
// MARK: - OpLedger
//
// The write-ahead ledger for every operation the engine runs (Copy,
...
// the ledger and knows exactly what was happening, what finished, and
// what was left mid-flight; and after a clean finish the ledger IS the
...
//             An Undo run's meta names the ledger it reverses
...
//             still parked — recovery must retry it, and the ledger
...
//             keep: landed work is real work, so a cancelled ledger is
//             both clean for recovery AND a valid undo candidate.
//   recovered Appended by the launch sweep once it has tidied an
//             interrupted ledger (idempotency marker).
//   undone    Appended to the ORIGINAL ledger when an undo of it
//             finishes clean, so it can never be undone twice.
//
// Retention is the v2 change of heart: a cleanly finished ledger is NOT
...
// non-dirty ledger (pruneSuperseded), which is simultaneously the
...
// beta stays visible in the media table via AvidLayout's temp-suffix
// rule either way.
class OpLedger
{
public:
	...
	/// Opens a fresh ledger and writes the begin line. FIRST prunes every
	/// finished, non-dirty ledger in the directory — see the retention
	/// note above; this is the one place undo is invalidated. `meta` goes
	/// in the begin line (dest root, preserve flag; undo: `undoes` +
	/// `effective`). `dir` defaults to the standard oplog location; tests
	/// pass a temp dir. Always check isOpen() before trusting protection.
	///
	/// `sparePath`: a ledger the prune must NOT touch. An Undo run passes
	/// the ledger it is reversing — pruning it would destroy the very
	...
	OpLedger(OpKind kind, const QJsonObject &meta, const QString &dir = QString(),
	...
	OpLedger(const OpLedger &) = delete;
	OpLedger &operator=(const OpLedger &) = delete;
	...
	/// Delete every schema-2 ledger in `dir` that is finished and not
	/// dirty — superseded undo candidates. Interrupted ledgers (no end
	...
	/// <app-data>/oplog, `MEDIAMUSTER_OPLOG_DIR` override first. Created
	/// on demand by the constructor.
	static QString standardOplogDir();

	/// Can a ledger actually be written right now? Probes with a scratch
	...
	/// The two ledger-unavailable warnings, worded once for every flow.
	static QString openFailedText(OpKind k);
	static QString degradedText();

	// MARK: - Read side

	/// One ledgered op, as read back for recovery or undo.
	struct Entry
	{
		int id = -1;
		QString src;
		...
		/// inside a ledger that carries an end line.
	...
	struct Record
	{
		...
		QString undoes;					   ///< The original ledger's file path... name.
	...
	/// Parse one ledger file. nullopt if it can't be opened, or if its
	/// begin line isn't schema 2 (legacy: not ours to interpret). Torn
	/// lines (crash mid-write) are skipped, never fatal.
	static std::optional<Record> readOne(const QString &ledgerPath);

	/// Parse every schema-2 oplog-*.jsonl in `dir` (standard dir when
	...
	/// The one run Edit ▸ Undo may reverse right now: the NEWEST ledger
	...
	/// ledger; makes the rollback idempotent (crash mid-rollback leaves
	/// it unstamped, so the next launch retries).
	static bool markRecovered(const QString &ledgerPath, int reversed, int failed);

	/// Appended to the ORIGINAL ledger when an undo of it finishes clean.
	/// `by` is the undo run's ledger file name, for forensics.
	static bool markUndone(const QString &ledgerPath, const QString &by);
...
// MARK: - LedgerOp
//
// RAII handle for one ledgered op. Build it right before touching disk
...
// ledger that closes clean. A real crash skips the destructor on
// purpose: the op stays unmarked, which is what recovery probes for.
//
// A null ledger (open failed) makes every call a no-op, so callers
// don't have to branch.
class LedgerOp
{
public:
	LedgerOp(OpLedger *ledger, const QString &src, const QString &dst, qint64 bytes,
			 const QString &parked, const FileIdentity &srcId,
			 const FileIdentity &parkedOriginalId = {})
		: m_ledger(ledger)
	{
		if (m_ledger)
			m_id = m_ledger->planOp(src, dst, bytes, parked, srcId, parkedOriginalId);
	}

	~LedgerOp()
	{
		if (m_ledger && !m_settled)
			m_ledger->markFailed(m_id, QStringLiteral("interrupted"));
	}

	LedgerOp(const LedgerOp &) = delete;
	LedgerOp &operator=(const LedgerOp &) = delete;

	void done(const OpLedger::DoneInfo &info = {})
	{
		if (m_ledger && !m_settled)
			m_ledger->markDone(m_id, info);
		m_settled = true;
	}
	...
	{
		if (m_ledger && !m_settled)
			m_ledger->markFailed(m_id, error);
		m_settled = true;
	}
	...
	/// the ledger alive past finish() so next-launch recovery can finish
	/// the job. Call when ParkedFile::isStranded() reports true.
	void failedDirty(const QString &error = QString())
	{
		if (m_ledger && !m_settled)
			m_ledger->markFailed(m_id, error, /*rollbackIncomplete=*/true);
		m_settled = true;
	}

	/// The op concluded with disk unchanged (conflict policy said Skip).
	/// The plan still lists the file, so the ledger has to say it was
	/// dealt with or a resume would offer it again.
	void skipped()
	{
		if (m_ledger && !m_settled)
			m_ledger->markSkipped(m_id);
		m_settled = true;
	}

private:
	OpLedger *m_ledger = nullptr;
	int m_id = -1;
