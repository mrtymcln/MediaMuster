...
{
	m_data.clear(); m_toc.clear(); m_dict.clear(); m_entries.clear(); m_views.clear();
...
bool BentoFile::checkLabel(QByteArrayView label, qint64 fileSize, QString &reason)
{
	...
	{
		reason = QStringLiteral("no Bento label at end of file"); return false;
	}
	...
	{
		reason = QStringLiteral("unsupported Bento label version %1.%2").arg(m_major).arg(minor); return false;
	}
	...
	if (m_tocOffset + m_tocLength != quint64(fileSize - 24))
	{
		reason = QStringLiteral("TOC arithmetic does not close (off %1 + len %2 != size %3 - 24)")
			.arg(m_tocOffset).arg(m_tocLength).arg(fileSize); return false;
	}
	if (m_tocLength > kMaxTailBytes)
	{
		reason = QStringLiteral("TOC exceeds the 64 MiB metadata limit"); return false;
	}
	if (m_major == 1 && m_tocLength % 24 != 0)
	{
		reason = QStringLiteral("Bento1 TOC length is not a multiple of 24"); return false;
	}
...
bool BentoFile::indexToc(QByteArrayView toc, QString &reason)
{
	...
	if (m_major == 1)
	{
		...
		for (qsizetype pos = 0; pos < toc.size(); pos += 24)
		{
			...
			Entry e;
			e.object = u32At(p); e.property = u32At(p + 4); e.type = u32At(p + 8);
			e.value = u32At(p + 12); e.length = u32At(p + 16); e.tocPos = m_tocOffset + pos;
			const quint16 flags = u16At(p + 22);
			e.immediate = flags & 1; e.continued = flags & 2;
			if (e.immediate)
			{
				if (e.length > 4) return fail(pos, QStringLiteral("immediate value longer than four bytes"));
	...
	else
	{
		...
		while (pos < toc.size())
		{
			...
			switch (code)
			{
				...
				case 2: case 5: case 6: size = 8; break;
				case 3: case 4: case 10: case 11: case 12: case 13: case 14: case 15: size = 4; break;
				case 7: case 8: size = 12; break;
				case 9: break;
				case 25: case 26: size = 16; break;
				default: return fail(start, QStringLiteral("unsupported opcode %1").arg(code));
			}
			const qsizetype blockEnd = qMin<qsizetype>(toc.size(), ((start / m_tocBlockSize) + 1) * m_tocBlockSize);
			if (size > blockEnd - pos) return fail(start, QStringLiteral("truncated opcode payload or buffer crossing"));
			const char *p = toc.data() + pos;
			pos += size;
			auto w = [&](int n) { return word(p + n * 4, m_containerBigEndian); };
			if (code >= 1 && code <= 3)
			{
				if (awaitingValue) return fail(start, QStringLiteral("object/property/type without a value"));
				if (code != 1 && !haveObject) return fail(start, QStringLiteral("first entry is not NewObject"));
				if (code == 1) { object = w(0); property = w(1); type = w(2); haveObject = true; }
				if (code == 2) { property = w(0); type = w(1); }
				if (code == 3) type = w(0);
				refs = 0; awaitingValue = allowGeneration = allowReference = true;
				continue;
			}
			if (!haveObject) return fail(start, QStringLiteral("value without object/property/type"));
			if (code == 4)
			{
				if (!allowGeneration) return fail(start, QStringLiteral("misplaced generation"));
				allowGeneration = false; continue;
			}
			if (code == 15)
			{
				if (!allowReference) return fail(start, QStringLiteral("misplaced reference list"));
				refs = w(0); allowReference = allowGeneration = false; continue;
			}
			Entry e;
			e.object = object; e.property = property; e.type = type;
			e.referenceList = refs; e.tocPos = m_tocOffset + start;
			...
			else if (code == 5 || code == 6) { e.value = w(0); e.length = w(1); }
			else
			{
			...
			if (m_entries.size() >= kMaxEntries) return fail(start, QStringLiteral("too many TOC value segments"));
			m_entries.append(e);
			awaitingValue = allowGeneration = allowReference = false;
		}
		if (awaitingValue) return fail(pos, QStringLiteral("missing final value"));
	}
	...
	{
		Entry &e = m_entries[i];
		if (!e.continued) continue;
		if (i + 1 >= m_entries.size()) return fail(e.tocPos - m_tocOffset, QStringLiteral("unterminated continued value"));
	...
	for (qsizetype i = 0; i < m_entries.size(); ++i)
		if (m_entries[i].continued) m_entries[i].nextSegment = int(i + 1);
	return true;
}

bool BentoFile::fetch(quint64 at, quint64 length, QByteArray &out) const
{
	if (at > quint64(std::numeric_limits<qint64>::max()) || length > quint64(std::numeric_limits<qint64>::max())) return false;
	if (!m_file.seek(qint64(at))) return false;
...
bool BentoFile::locateLabel(qint64 fileSize, QByteArray &label, quint64 &labelEnd, QString &reason)
{
	...
	{
		if (offset > quint64(fileSize) || length > quint64(fileSize) - offset) return false;
		if (m_tailFirst) return fetch(offset, length, out);
		out = m_data.sliced(qsizetype(offset), qsizetype(length));
		return true;
	};
	if (!at(fileSize - 24, 24, label)) { reason = QStringLiteral("short read of Bento label"); return false; }
	labelEnd = quint64(fileSize);
	if (std::memcmp(label.constData(), kMagic, 8) == 0) return true;
	...
	if (!rf64)
	{
		...
		if (end > quint64(fileSize) || end < 12) { reason = QStringLiteral("invalid RIFF extent"); return false; }
	}
	quint64 pos = 12;
	constexpr int kMaxChunks = 65536;
	for (int chunks = 0; chunks < kMaxChunks && pos <= end && end - pos >= 8; ++chunks)
	{
		if (!at(pos, 8, header)) { reason = QStringLiteral("short read of RIFF chunk"); return false; }
		...
		{
			if (!haveDataLength) { reason = QStringLiteral("RF64 data has no preceding ds64 length"); return false; }
			length = dataLength;
		}
		if (length > end - body) { reason = QStringLiteral("RIFF chunk extends beyond container"); return false; }
		const quint64 padded = length + (length & 1);
		if (padded > end - body) { reason = QStringLiteral("missing RIFF chunk padding"); return false; }
		if (rf64 && id == "ds64")
		{
			QByteArray sizes;
			if (length < 28 || !at(body, 16, sizes)) { reason = QStringLiteral("malformed RF64 ds64 chunk"); return false; }
			const quint64 declared = qFromLittleEndian<quint64>(sizes.constData());
			if (declared > quint64(fileSize) - 8 || declared + 8 < body + padded)
			{ reason = QStringLiteral("invalid RF64 extent"); return false; }
			end = declared + 8;
		...
		if (id == "omfi" && padded >= 24)
		{
			...
			if (!at(labelEnd - 24, 24, label)) { reason = QStringLiteral("short read of RIFF Bento label"); return false; }
			if (std::memcmp(label.constData(), kMagic, 8) == 0) return true;
		}
		pos = body + padded;
	}
...
{
	reset(); m_bytesRead = data.size();
	QString reason;
	if (data.size() < 24) reason = QStringLiteral("too small for a Bento label");
	else
	{
		m_data = data;
		QByteArray label;
...
bool BentoFile::open(const QString &path, QString *why)
{
	...
	auto fail = [&]() { if (why) *why = reason; reset(); return false; };
	if (!m_file.open(QIODevice::ReadOnly)) { reason = QStringLiteral("cannot open: %1").arg(m_file.errorString()); return fail(); }
	const qint64 size = m_file.size();
	if (size < 24) { reason = QStringLiteral("too small for a Bento label"); return fail(); }
	QByteArray label;
	quint64 labelEnd = 0;
	m_tailFirst = true;
	if (!locateLabel(size, label, labelEnd, reason) || !checkLabel(label, qint64(labelEnd), reason)) return fail();
	if (!fetch(m_tocOffset, m_tocLength, m_toc)) { reason = QStringLiteral("short read of TOC"); return fail(); }
	if (!indexToc(m_toc, reason)) return fail();
	...
	{
		if ((e.property != 23 && e.property != 24) || e.immediate) continue;
		if (e.value > m_tocOffset || e.length > m_tocOffset - e.value) continue;
		lo = qMin(lo, e.value); hi = qMax(hi, e.value + e.length);
	}
	if (hi > lo && hi - lo <= kMaxTailBytes)
	{
		m_dictOffset = lo;
		if (!fetch(lo, hi - lo, m_dict)) { reason = QStringLiteral("short read of name dictionary"); return fail(); }
	}
	if (!indexNames(reason)) return fail();
	if (why) why->clear();
	return true;
}
...
bool BentoFile::indexNames(QString &reason)
{
	...
	for (const Entry &e : m_entries)
	{
		if (e.property != 24) continue;
		const ReadResult result = read(e.object, 24);
		if (!result.ok()) { reason = QStringLiteral("unreadable property-name dictionary entry %1").arg(e.object); return false; }
		total += result.data.size();
		if (total > kMaxTailBytes) { reason = QStringLiteral("property-name dictionary exceeds 64 MiB"); return false; }
		const QByteArray name = untilNul(result.data).toByteArray();
		if (!name.isEmpty() && !m_propIdByName.contains(name)) m_propIdByName.insert(name, int(e.object));
	}
	...
	if (version.isEmpty()) version = bytes(1, propertyId("OMFI:Version"));
	...
	if (order == "MM") m_metadataBigEndian = true;
	else if (order == "II") m_metadataBigEndian = false;
	return true;
}
...
BentoFile::ReadResult BentoFile::read(quint32 object, int property, qint64 cap) const
{
	...
	if (!first) return {{}, ReadStatus::Missing};
	quint64 total = 0;
	for (const Entry *e = first; e; e = e->nextSegment < 0 ? nullptr : &m_entries[e->nextSegment])
	{
		if (e->immediate ? e->length > quint64(e->immediateData.size()) :
			(e->value > m_tocOffset || e->length > m_tocOffset - e->value)) return {{}, ReadStatus::Malformed};
		if (cap < 0 || e->length > quint64(cap) - total) return {{}, ReadStatus::TooLarge};
		total += e->length;
	}
	QByteArray out; out.reserve(qsizetype(total));
	for (const Entry *e = first; e; e = e->nextSegment < 0 ? nullptr : &m_entries[e->nextSegment])
	{
		if (e->immediate) out += e->immediateData;
		else if (!m_tailFirst) out.append(m_data.constData() + e->value, qsizetype(e->length));
		...
		{
			QByteArray part;
			if (!fetch(e->value, e->length, part)) return {{}, ReadStatus::IoError};
			out += part;
		}
	}
...
QByteArrayView BentoFile::value(quint32 object, int property) const
{
	...
	if (it == m_views.cend()) it = m_views.insert(key, bytes(object, property));
...
QVector<quint32> BentoFile::objectsWithProperty(int property) const
{
	...
	for (const Entry &e : m_entries)
		if (property >= 0 && e.property == quint32(property) && (result.isEmpty() || result.last() != e.object)) result.append(e.object);
	return result;
}

quint32 BentoFile::uintValue(QByteArrayView v) const
{
	if (v.size() == 1) return quint8(v[0]);
	if (v.size() == 2) return half(v.data(), m_metadataBigEndian);
	if (v.size() == 4) return word(v.data(), m_metadataBigEndian);
	return 0;
}
quint64 BentoFile::uint64Value(QByteArrayView v) const
{
	if (v.size() != 8) return uintValue(v);
...
{
	if (v.size() == 4) return qint32(uintValue(v));
	if (v.size() == 2) return qint16(uintValue(v));
...
bool BentoFile::rationalValue(QByteArrayView v, qint32 &num, qint32 &den) const
{
	...
	num = qint32(uintValue(v.first(4))); den = qint32(uintValue(v.sliced(4))); return true;
}
quint32 BentoFile::handleValue(QByteArrayView v) const
{
	if (v.size() != (m_omf2References ? 4 : 8)) return 0;
...
QVector<quint32> BentoFile::handlesValue(QByteArrayView v) const
{
	...
	if (v.size() != 2 + qsizetype(count) * stride) return out;
	for (quint32 i = 0; i < count; ++i) { const quint32 id = handleValue(v.sliced(2 + i * stride, stride)); if (id) out.append(id); }
	return out;
}
quint32 BentoFile::mappedReference(const Entry &e, QByteArrayView raw, ReadStatus &status) const
{
	if (raw.size() != 4) { status = ReadStatus::Malformed; return 0; }
	...
	auto existingObject = [&](quint32 id)
	{
		const auto it = std::lower_bound(m_entries.cbegin(), m_entries.cend(), id,
			[](const Entry &candidate, quint32 target) { return candidate.object < target; });
		if (it == m_entries.cend() || it->object != id) { status = ReadStatus::Malformed; return quint32(0); }
		return id;
	};
	if (m_major == 1 || e.referenceList == 0) return existingObject(key);
	const Entry *recording = find(e.referenceList, 31);
	if (!recording || recording->type != 32) { status = ReadStatus::Malformed; return 0; }
	const ReadResult table = read(e.referenceList, 31);
	if (!table.ok()) { status = table.status == ReadStatus::Missing ? ReadStatus::Malformed : table.status; return 0; }
	if (table.data.size() % 8 != 0) { status = ReadStatus::Malformed; return 0; }
	...
	status = ReadStatus::Malformed; return 0;
}
quint32 BentoFile::ref(quint32 object, int property, ReadStatus *status) const
{
	const ReadResult r = read(object, property);
	ReadStatus s = r.status; quint32 result = 0;
	if (r.ok())
	{
		if (r.data.size() != (m_omf2References ? 4 : 8)) s = ReadStatus::Malformed;
		else result = mappedReference(*find(object, quint32(property)), QByteArrayView(r.data).first(4), s);
	}
	if (status) *status = s;
	return result;
}
