...
/// Works around a framework 6.5 crash in the macOS accessibility bridge.
///
/// THE BUG. The Cocoa platform plugin keeps its own array of a
...
/// the throw crosses the framework's C++ frames the process aborts. Seen in the field
...
/// -[__NSArrayM objectAtIndexedSubscript:]. The bridge fix rebuilds the
/// table model when an out-of-bounds cell is requested; the version gate
/// below disables this guard once that fix is available.
///
/// THE GUARD. Item views expose a "table" accessibility interface; that is
/// what makes the bridge build per-row elements at all. On macOS, with a framework
...
/// this factory is asked before the default factory, so it wins for every
...
/// user's log.
///
/// RETIRES ITSELF. The whole thing compiles to nothing on a framework that
/// carries the fix (>= 6.6.2) and on Windows. Delete this file when the
/// framework pin moves past 6.6.2.
