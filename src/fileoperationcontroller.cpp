											  ...
											  const QVector<OpTrashFallbackItem> &items)
{
	...
	auto *parent = m_progressDialog && m_progressDialog->isVisible()
					   ? static_cast<QWidget *>(m_progressDialog) : m_window;
	...
	dialog->setInformativeText(tr("The system bin couldn’t accept these files. Keep them in MediaMuster Trash until you decide."));
	QStringList details;
	...
	auto *move = dialog->addButton(tr("Move to Trash"), QMessageBox::AcceptRole);
