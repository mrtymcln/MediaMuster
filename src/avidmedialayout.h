...
namespace AvidMediaLayout
{
	...
	[[nodiscard]] inline bool isReservedFolderName(QStringView name)
	{
		return Conventions::isDotHidden(name) || Conventions::isCreatingFolderName(name) ||
			   name.compare(QLatin1String("Temp"), Qt::CaseInsensitive) == 0 ||
			   name.compare(QLatin1String("Quarantine"), Qt::CaseInsensitive) == 0 ||
			   name.compare(QLatin1String("Quarantined Files"), Qt::CaseInsensitive) == 0;
	}

	[[nodiscard]] inline std::optional<MxfFolderName> parseMxfFolderName(QStringView name)
	{
		if (!Detail::isLeafName(name) || isReservedFolderName(name))
			return std::nullopt;
	...
	[[nodiscard]] inline std::optional<Location> locateMediaFolder(const QString &path)
	{
		...
		if (isMxfRoot(parent) && parseMxfFolderName(name))
			return Location{Family::Mxf, parent, name};
		if (isOmfRoot(parent) && Detail::isLeafName(name) && !isReservedFolderName(name))
