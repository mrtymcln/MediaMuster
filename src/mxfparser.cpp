...
MxfMetadata MxfParser::parseFromBuffer(const QByteArray &data)
{
	...
	{
		const QByteArray key = data.mid(pos, 16);
		int bytesUsed = 0;
		...
		if (std::memcmp(key.constData(), UL_SET_PREFIX, 13) == 0)
		{
			const auto setType = static_cast<quint8>(key[14]);
			switch (setType)
			{
			case SET_CDCI:
			case SET_RGBA:
...
QString MxfParser::codecFromEssenceLabel(const QByteArray &label, const QString &fps)
{
	...
	{
		if (fps == "29.97" || fps == "30")
			return "DNxHD LB (DNxHD 45)";
		if (fps == "50" || fps == "59.94")
			return "DNxHD LB (DNxHD 90)";
		return "DNxHD LB (DNxHD 36)";
	}
	if (baseName == "DNxHD SQ")
	{
		if (fps == "29.97" || fps == "30")
			return "DNxHD SQ (DNxHD 145)";
		if (fps == "25")
			return "DNxHD SQ (DNxHD 120)";
		return "DNxHD SQ (DNxHD 115)";
	}
	if (baseName == "DNxHD HQ")
	{
		if (fps == "29.97" || fps == "30")
			return "DNxHD HQ (DNxHD 220)";
		if (fps == "25")
			return "DNxHD HQ (DNxHD 185)";
		return "DNxHD HQ (DNxHD 175)";
	}
	if (baseName == "DNxHD HQX")
	{
		if (fps == "29.97" || fps == "30")
			return "DNxHD HQX (DNxHD 220X)";
		if (fps == "25")
			return "DNxHD HQX (DNxHD 185X)";
		return "DNxHD HQX (DNxHD 175X)";
	}
