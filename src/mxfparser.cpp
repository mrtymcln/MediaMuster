...
// KLV parsing. Only the header partition is read — we never
// touch the essence, so scan time is independent of file size.
//
// MARK: - Anchor labels
//
// The two SMPTE Universal Labels below are the KLV keys we anchor
// against. `UL_HEADER_PARTITION` marks where parsing starts;
// `UL_SET_PREFIX` flags every metadata Set we care about
...
// Set type bytes — only the last byte of the UL differs between Sets.
// We compare the first 13 bytes against `UL_SET_PREFIX` and then
// switch on byte 14.
...
/// Read a duration value, which comes in either 4- or 8-byte big-
/// endian form depending on the MXF flavour. Caller must have already
/// bounds-checked that `pos + len` fits inside the buffer.
...
MxfMetadata MxfParser::parseHeader(const QString &filePath, qint64 *bytesRead)
{
	...
	// Re-read up to `kFallbackRead` only if the first pass came back
	// invalid AND more data remains on disk — otherwise we'd reparse
	// the same bytes for files smaller than `kFastRead`.
	if (!meta.valid && data.size() < kFallbackRead && data.size() < file.size())
	{
...
MxfMetadata MxfParser::parseFromBuffer(const QByteArray &data)
{
	...
	// Interlaced sources store one field height in the descriptor
	// (e.g. 540 for 1080i). FrameLayout 1 (Separate Fields) and 3
	// (Mixed Fields) both need doubling to get the full-frame height.
	if (meta.frameLayout == 1 || meta.frameLayout == 3)
		meta.height *= 2;

	// Some Avid MXFs write garbage into width tag 0x3203 — zero
	// out implausible values so height-only inference can run.
	if (meta.width > 16384)
		meta.width = 0;
	...
	else if (meta.height > 0)
	{
		...
		// sizes. Covers UHD, HD, NTSC and PAL SD shapes.
	...
	// + height) rather than the codec UL itself.
		...
		case 0x3001: // sample rate — fps for video, Hz for audio
			if (len >= 8)
			{
				...
				if (den > 0)
				{
					...
					{
						// Recognise the standard NTSC pulldowns
						// explicitly so the display reads "29.97"
...
void MxfParser::parseMaterialPackage(const QByteArray &data, qint64 startPos, qint64 length, MxfMetadata &out)
{
	...
	while (pos + 4 <= endPos)
	{
		...
		if (tag == 0x4401 && len >= 32 && out.umid.isEmpty())
		{
			...
			out.umid = MobId::format(
			    reinterpret_cast<const unsigned char *>(data.constData() + pos));
		}
...
/// Maps essence-container ULs to display codec names. All ULs here
/// have been verified against real files.
QString MxfParser::codecFromEssenceLabel(const QByteArray &label, const QString &fps)
{
	...
	// DNxHD bitrate names depend on framerate — the UL only
	// identifies the quality tier.
	struct DnxEntry
	{
		const char *tier, *r30, *r25, *r60, *rDefault;
	};
