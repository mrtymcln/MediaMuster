...
OmfMetadata OmfParser::parseHeader(const QString &filePath, qint64 *bytesRead)
{
	...
	for (quint32 obj : b.objectsWithProperty(p.mobId))
	{
		...
		if (it->usageCode < 0 && !usage.isEmpty())
			it->usageCode = int(b.uintValue(usage));
	...
	for (const QString &hex : order)
	{
		...
		if (p.omf2 ? !g.masterClass :
			(g.anyPhysical || (p.revision == OmfObjects::Revision::Omf1 && g.usageCode != 7 && g.usageCode != 1)))
			continue;
	...
	if (master)
	{
		...
		e.classificationKnown = master->usageCode == 1 || master->usageCode == 7;
	}
